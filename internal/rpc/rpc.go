...
func InitChainServiceAndRunRpcServer(pkString string, chainOpts chain.ChainOpts,
	...
	rpcServer, node, messageService, err := RunRpcServer(pk, chainService, useDurableStore, durableStoreFolder, msgPort, wsMsgPort, rpcPort, transportType, os.Stdout, bootPeers, useMdns)
	if err != nil {
		return nil, nil, nil, err
	}
...
func RunRpcServer(pk []byte, chainService chainservice.ChainService,
	...
	messageService := p2pms.NewMessageService("127.0.0.1", msgPort, wsMsgPort, *ourStore.GetAddress(), pk, true, logDestination, bootPeers)
	node := node.New(
		messageService,
		chainService,
		ourStore,
