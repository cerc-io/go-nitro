...
func closeLedgerChannel(t *testing.T, alpha node.Node, beta node.Node, channelId types.Destination, isChallengeInitiatedByMe bool) {
	response, err := alpha.CloseLedgerChannel(channelId, isChallengeInitiatedByMe)
	if err != nil {
		t.Fatal(err)
	}
