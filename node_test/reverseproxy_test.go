...
func setupNitroClients(t *testing.T, logDestination *os.File) (alice, irene, bob rpc.RpcClientApi, cleanup func()) {
	...
	ireneClient, msgIrene, ireneCleanup := setupNitroNodeWithRPCClient(t, ta.Irene.PrivateKey, 3106, 5106, 4106, ireneChainService, logDestination, "ws", []string{})
	bootPeers := []string{msgIrene.MultiAddr}
	aliceClient, msgAlice, aliceCleanup := setupNitroNodeWithRPCClient(t, ta.Alice.PrivateKey, 3105, 5105, 4105, aliceChainService, logDestination, "ws", bootPeers)

	bobClient, msgBob, bobCleanup := setupNitroNodeWithRPCClient(t, ta.Bob.PrivateKey, 3107, 5107, 4107, bobChainService, logDestination, "ws", bootPeers)
