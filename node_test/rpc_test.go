...
func executeNRpcTest(t *testing.T, connectionType transport.TransportType, n int, manualVoucherExchange bool) {
	...
	for i := 0; i < n; i++ {
		rpcClient, msg, cleanup := setupNitroNodeWithRPCClient(t, actors[i].PrivateKey, 3105+i, 5105+i, 4105+i, chainServices[i], logDestination, connectionType)
		clients[i] = rpcClient
		msgServices[i] = msg
		defer cleanup()
	}
...
func setupNitroNodeWithRPCClient(
	...
	var err error
	rpcServer, _, messageService, err := interRpc.RunRpcServer(pk, chain, false, "", msgPort, wsMsgPort, rpcPort, connectionType, logDestination, []string{}, true)
	if err != nil {
		t.Fatal(err)
	}
